...
export def main [target_hostname: string]: nothing -> nothing {
    ...
    let command = $"ls -s /dev/disk/by-id/ | get name | to text | fzf --header 'Select boot disk' | save /tmp/boot_disk; print 'Scanning Hardware'; sudo nixos-facter -o ($remote_facter);"
