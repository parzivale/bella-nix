_:
let
  # Class-neutral: the driver is a kernel module built against whatever
  # `boot.kernelPackages` is, and both classes have that option.
  driverFor =
    config:
  ...
  driver =
    { config, ... }:
    {
      boot.extraModulePackages = [ (driverFor config) ];
      boot.kernelModules = [ "r8126" ];
    };
in
{
  # r8169 (in-tree) probes the RTL8126 too and wins the race if not blocked, so
  # it has to be blacklisted for r8126 to actually bind. That is the only part
  # that differs: nixos spells it `boot.blacklistedKernelModules`, and finix
  # keeps it with the thing that reads it - modprobe.d - while accepting the
  # nixos name as a renamed alias.
  flake.modules.nixos.r8126 = {
    imports = [ driver ];
    boot.blacklistedKernelModules = [ "r8169" ];
  };

  flake.modules.finix.r8126 = {
    imports = [ driver ];
    programs.modprobe.blacklist = [ "r8169" ];
  };
}
