  ...
  flake.modules.nixos.hookshot =
    ...
    let
      hookshot-src = pkgs.fetchFromGitHub {
        ...
        rev = "34fafb03541e102fb2d40b98d941206a48ba9e8b";
        hash = "sha256-DUWJ/f9ONV7pSHeKfRbwKJGMFIBeyHbXP596a1d9F/U=";
      };
      hookshot = pkgs.matrix-hookshot.overrideAttrs (old: {
        src = hookshot-src;
        version = "7.3.3-unstable-2026-05-19";
        ...
        cargoDeps = pkgs.rustPlatform.fetchCargoVendor {
          ...
          version = "7.3.3-unstable-2026-05-19";
          src = hookshot-src;
          hash = "sha256-smpAU2ndA/Lh+PaiAvVjzUy6grwCvZldT4WaSXrWeqM=";
        };
        # build:app:fix-defs was removed upstream; defs are now generated by build:app:rs directly
        buildPhase = ''
          runHook preBuild
          yarn run build:app:rs --target ${pkgs.stdenv.hostPlatform.rust.rustcTarget}
          yarn run build:app
          yarn run build:web
          runHook postBuild
        '';
    ...
    in
    {
      ...
      systemd.services.matrix-hookshot = {
        ...
        after = [
          "network.target"
          "matrix-synapse.service"
          "matrix-hookshot-gen-registration.service"
        ];
        requires = [
          "matrix-synapse.service"
          "matrix-hookshot-gen-registration.service"
