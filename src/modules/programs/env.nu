# let bash-env-json = ${pkgs.bash-env-json}/bin/bash-env-json
export def bash-env [
    path?: string
    --export: list<any>
    --shellvars(-s)
    ...
    if $error != null { error make {msg: $error} } else if $raw.exit_code != 0 { error make {
        msg: $"unexpected failure from bash-env-json ($raw.stderr)"
    } }
    ...
    } else if $shellvars or ($fn | is-not-empty) { $raw_json } else { $raw_json.env }
) }
bash-env
